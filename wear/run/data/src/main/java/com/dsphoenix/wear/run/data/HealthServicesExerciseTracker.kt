...
class HealthServicesExerciseTracker(
    ...
    override suspend fun isHeartRateTrackingSupported(): Boolean {
        val isTrackingSupported = runCatching {
        ...
        return hasBodySensorsPermission() && isTrackingSupported
    }
    ...
    override suspend fun stopExercise(): EmptyResult<ExerciseError> {
        val result =
            checkExerciseStatePreconditions(errors = setOf(ExerciseError.ONGOING_OTHER_EXERCISE))
        if (result is Result.Error)
            return result
    ...
    // Success if no errors or the error is not specified
    private suspend fun checkExerciseStatePreconditions(errors: Set<ExerciseError> = setOf()): EmptyResult<ExerciseError> {
        ...
        if (result is Result.Error) {
            if (errors.isEmpty() || result.error in errors) {
                return result
            }
        }
